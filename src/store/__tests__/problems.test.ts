...
import { type LeetCodeQuestion, type Question } from '../problem';
...
describe('test machines', () => {
  ...
  function create() {
    ...
    // records past
    const newPast = problemsState.context.problems.length
      ? [...problemsState.context.past, [...problemsState.context.problems]]
      : [[]];
    ...
    problemsState = problemsActor.getSnapshot();
    const problemActor =
      problemsState.context.problemMachines[
        problemsState.context.problemMachines.length - 1
      ];
    problemActor.start();
    ...
    expect(problemsState.context.problems.map(removeReviews)).toEqual(
      newProblems.map(removeReviews),
    );
  ...
  function undo() {
    problemsState = problemsActor.getSnapshot();
    const newPast = problemsState.context.past.slice(
      0,
      problemsState.context.past.length - 1,
    );
    const newProblems =
      problemsState.context.past[problemsState.context.past.length - 1];
    const nextFurture = [
      problemsState.context.problems,
      ...problemsState.context.furture,
    ];
  ...
  function train(problemIdx = getRandomQuestionIdx()) {
    ...
    const newProblems = [
      ...problemsState.context.problems.slice(0, problemIdx),
      newProblem,
      ...problemsState.context.problems.slice(problemIdx + 1),
    ];
    ...
    expect(newProblemContext.reviews.slice(0, -1)).toEqual(
      newProblems[problemIdx].reviews.slice(0, -1),
    );
    expect(
      newProblemContext.reviews[newProblemContext.reviews.length - 1] >=
        newProblems[problemIdx].reviews[
          newProblems[problemIdx].reviews.length - 1
        ],
    ).toEqual(true);
    expect(problemsState.context.furture).toEqual(nextFurture);
  }
  ...
  test('create a problem', () => {
    ...
    const randomIdxs = Array.from(
      new Set(
        Array(50)
          .fill(0)
          .map((v) => getRandomQuestionIdx(size)),
      ),
    );
    ...
    randomIdxs.forEach((idx) => {
      ...
      expect(prev.problemMachines.map((v) => v.toJSON?.())).toEqual(
        next.problemMachines.map((v) => v.toJSON?.()),
      );
